    ...
    foreach($res as $res)
    {
        if($res['product_id']==$res['animal_id']){
           $tot+=$res['price']*$res['quantity']; 
            echo "<span>".$res['animal_name']." ".$res['animal_price']." x".$res['quantity']."</span><form action='removeCart.php'><input type='hidden' name='cid' value=".$res['cart_id']."><input type='hidden' name='aid' value=".$res['animal_id']."><input type='submit'></form></br>";
        }
       /*if($res['product_id']==$res['animal_id']){
